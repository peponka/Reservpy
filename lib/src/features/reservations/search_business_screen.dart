...
// ─── Local state: selected category for drill-down ───────────────────────────
final _selectedCategoryProvider = StateProvider<BusinessCategory?>((ref) => null);

/// Two-step search screen:
///   Step 1 — Browse categories (big card grid)
///   Step 2 — View businesses in the selected category
...
class _SearchBusinessScreenState extends ConsumerState<SearchBusinessScreen> {
  ...
  Color _gradientColor(String name) {
    final hash = name.hashCode;
    final hue = (hash % 360).toDouble().abs();
    return HSLColor.fromAHSL(1, hue, 0.5, 0.4).toColor();
  }
...
class _CategoriesView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Sort alphabetically, keep "Otros" last
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'Encontrá el lugar perfecto para tu próxima reserva',
            style: GoogleFonts.inter(
              fontSize: 14,
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          transform:
              Matrix4.translationValues(0, _isHovered ? -4 : 0, 0),
          decoration: BoxDecoration(
            color: _isHovered
...
// ═════════════════════════════════════════════════════════════════════════════
// STEP 2: BUSINESSES LIST FOR SELECTED CATEGORY
// ═════════════════════════════════════════════════════════════════════════════
class _BusinessListView extends ConsumerStatefulWidget {
  final BusinessCategory category;
  final TextEditingController searchController;
  final FocusNode searchFocusNode;
  final Color Function(String) gradientColor;
  ...
  @override
  ConsumerState<_BusinessListView> createState() =>
      _BusinessListViewState();
}

class _BusinessListViewState extends ConsumerState<_BusinessListView> {
              ...
              actions: [
                ...
                ElevatedButton.icon(
                  onPressed: () async {
                    ...
                    final hue =
                        (name.hashCode % 360).toDouble().abs();
                    final catColor =
                        HSLColor.fromAHSL(1, hue, 0.55, 0.45)
                            .toColor();
                    final hexColor = '#${catColor.toARGB32().toRadixString(16).padLeft(8, '0').substring(2)}';
        ...
        return SingleChildScrollView(
          child: ConstrainedBox(
            ...
            child: Column(
              ...
              children: [
                // ── Category header ──
                _buildCategoryHeader(context, widget.category),
                ...
                Padding(
                  ...
                  child: Container(
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        hintText:
                            'Buscar en ${widget.category.name}...',
                        hintStyle: GoogleFonts.inter(
                            fontSize: 14,
                            color: AppColors.textSecondary),
                    ...
                    child: GridView.builder(
                      ...
                      itemBuilder: (context, index) {
                        return _BusinessCard(
                          ...
                          gradientColor:
                              widget.gradientColor(filtered[index].name),
                        );
                      },
                    ),
                  ),
  ...
  Widget _buildCategoryHeader(
      BuildContext context, BusinessCategory category) {
    return Container(
      margin: const EdgeInsets.fromLTRB(
          AppSizes.s24, AppSizes.s16, AppSizes.s24, 0),
  ...
  Widget _buildEmptyState(
      BuildContext context, BusinessCategory category) {
    final isOtros = category.id == 'cat-25';
                ...
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    colors: [
                      ...
                      widget.gradientColor.withValues(alpha: 0.7),
                    ],
                    begin: Alignment.topLeft,
                    end: Alignment.bottomRight,
                  ),
              ...
              Expanded(
                child: Padding(
                  padding: const EdgeInsets.fromLTRB(
                      AppSizes.s16, AppSizes.s16, AppSizes.s16, AppSizes.s12),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(
                      ...
                      Row(
                        children: [
                          ...
                          Container(
                            padding: const EdgeInsets.symmetric(
                                horizontal: AppSizes.s6, vertical: 2),
                            decoration: BoxDecoration(
                              color:
